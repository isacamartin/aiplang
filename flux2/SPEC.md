# FLUX v2.0 — Full-Stack AI-First Language Specification

## Philosophy

FLUX v2 is a complete full-stack language.
...
Same as FLUX v1 + new reactive bindings:
...
# Production
fluxd build app.flux            # compile to optimized Go binary
fluxd start app.flux            # start production server
fluxd migrate app.flux          # run DB migrations only
fluxd migrate app.flux --reset  # drop and recreate tables

# Deploy
fluxd deploy app.flux           # deploy to Fly.io / Railway / Render
```

---

## Output — what fluxd generates
...
Same app as above:
- FLUX v2:   ~600 tokens (frontend + backend + DB)
