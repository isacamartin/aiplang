...
func renderPageHTML(page fc.Page, allPages []fc.Page) string {
	...
	if needsJS {
		...
		hydrateTag = fmt.Sprintf("\n<script>window.__FLUX_PAGE__=%s;</script>\n<script src=\"/static/flux-hydrate.js\" defer></script>", config)
	}
...
func getBaseCSS(theme string) string {
	// Same CSS as flux-web npm package — omitted here for brevity
