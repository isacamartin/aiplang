...
if (!cmd || cmd === '--help' || cmd === '-h') {
  console.log(`
  flux-web v${VERSION}
  AI-first web language — full CRUD apps in ~20 lines.

  Usage:
    npx flux-web init [name]        create a new project
    npx flux-web serve [dir]        dev server → localhost:3000
    npx flux-web build [dir/file]   compile .flux → static HTML
    npx flux-web new <page>         create a new .flux page
...
if (cmd === '--version' || cmd === '-v') {
  console.log(`flux-web v${VERSION}`)
  process.exit(0)
}
...
row3{rocket>Fast>Renders in under 1ms. Zero framework overhead.|bolt>AI-native>Written by Claude in seconds.|globe>Deploy anywhere>Static files. Any host. Zero config.}
...
if (cmd === 'build') {
  ...
  if (fs.existsSync('public')) {
    fs.readdirSync('public').filter(f => !f.endsWith('.flux')).forEach(f =>
...
if (cmd === 'serve' || cmd === 'dev') {
  ...
  require('http').createServer((req, res) => {
    ...
    let fp = null
    for (const c of [path.join(root,'public',p), path.join(root,p)]) {
      if (fs.existsSync(c) && fs.statSync(c).isFile()) { fp = c; break }
    }
    if (!fp && p.endsWith('.flux')) {
      const c = path.join(root, 'pages', path.basename(p)); if (fs.existsSync(c)) fp = c
    }
  ...
  }).listen(port, () => console.log(`\n  ✓  flux-web dev server\n\n  →  http://localhost:${port}\n\n  Edit pages/*.flux and reload. Ctrl+C to stop.\n`))
  return
}
...
function parseBlock(line) {
  // table with optional edit/delete
  if (line.startsWith('table ')) {
    ...
    const deleteKey   = (content.match(/delete-key:(\S+)/) || [])[1] || 'id'
    const clean = content.replace(/edit\s+(PUT|PATCH)\s+\S+/g,'').replace(/delete\s+(?:DELETE\s+)?\S+/g,'').replace(/delete-key:\S+/g,'')
    return { kind:'table', binding, cols:parseCols(clean), empty:parseEmpty(clean),
      ...
      deletePath:deleteMatch?.[1]||null, deleteKey }
  }
  // form
  ...
  // btn{Label > METHOD /path > action}
  if (line.startsWith('btn{') || line.startsWith('btn ')) {
    const bi = line.indexOf('{'); if (bi === -1) return null
    const parts = line.slice(bi + 1, line.lastIndexOf('}')).split('>').map(p => p.trim())
...
function parseCols(s) {
  return s.split('|').map(c => {
    c = c.trim(); if (c.startsWith('empty:') || !c) return null
...
function renderPage(page, allPages) {
  const needsJS = page.queries.length > 0 ||
    page.blocks.some(b => ['table','list','form','if','btn','select'].includes(b.kind))
  ...
  const config = needsJS ? JSON.stringify({
    ...
    state:page.state, queries:page.queries
  }) : ''
...
function rTable(b) {
  ...
  const deleteAttr = b.deletePath ? ` data-fx-delete="${esc(b.deletePath)}" data-fx-delete-key="${esc(b.deleteKey)}"` : ''
  const actTh      = (b.editPath||b.deletePath) ? '<th class="fx-th fx-th-actions">Actions</th>' : ''
  const colSpan    = b.cols.length + ((b.editPath||b.deletePath)?1:0)
  return `<div class="fx-table-wrap"><table class="fx-table" data-fx-table="${esc(b.binding)}" data-fx-cols='${keys}' data-fx-col-map='${colMap}'${editAttr}${deleteAttr}><thead><tr>${ths}${actTh}</tr></thead><tbody class="fx-tbody"><tr><td colspan="${colSpan}" class="fx-td-empty">${esc(b.empty)}</td></tr></tbody></table></div>\n`
}
...
function rBtn(b) {
  const confirmAttr = b.confirm ? ` data-fx-confirm="${esc(b.confirm)}"` : ''
  const actionAttr  = b.action  ? ` data-fx-action="${esc(b.action)}"` : ''
  return `<div class="fx-btn-wrap"><button class="fx-btn fx-standalone-btn" data-fx-btn="${esc(b.bpath)}" data-fx-method="${esc(b.method)}"${actionAttr}${confirmAttr}>${esc(b.label)}</button></div>\n`
}
